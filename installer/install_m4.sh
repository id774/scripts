...
########################################################################
# install_m4.sh: Installer for GNU M4
#
#  Description:
#  This script automates the installation of GNU M4 by:
...
#
#  Usage:
#  Run this script without arguments to install the default version (1.4.16):
#      ./install_m4.sh
#  Specify a version to install a different release:
#      ./install_m4.sh 1.4.19
#  Skip saving sources by adding a second argument:
#      ./install_m4.sh 1.4.19 -n
#
#  Requirements:
...
install_m4() {
    setup_environment "$1"
    mkdir install_m4
    cd install_m4 || exit 1
    wget "ftp://ftp.gnu.org/gnu/m4/m4-$VERSION.tar.gz"
    if [ ! -f "m4-$VERSION.tar.gz" ]; then
        echo "Error: Failed to download M4 $VERSION." >&2
        exit 1
    fi
