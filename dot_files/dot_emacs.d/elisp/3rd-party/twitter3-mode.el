...
(defconst twitter3-http-status-line-regexp
...
(defvar twitter3-timer-interval 101)
...
(defvar twitter3-process-info-alist nil
...
(if twitter3-mode-map
    (let ((km twitter3-mode-map))
      ...
      (define-key km "i" 'twitter3-other-user-timeline)
      ...
      (define-key km "t" 'twitter3-icon-mode)
      (define-key km "s" 'twitter3-scroll-mode)
      (define-key km "a" 'twitter3-favorite)
      (define-key km "\C-c\C-a" 'twitter3-favorite)
      (define-key km "q" 'twitter3-unfavorite)
      nil))
...
(defun twitter3-http-post-new (host method &optional parameters format sentinel)
...
FORMAT is a response data format (\"xml\", \"atom\", \"json\")"
  (if (null format)
      (setq format "xml"))
  (if (null sentinel)
      (setq sentinel 'twitter3-http-post-default-sentinel-new))
...
(defun twitter3-http-default-sentinel-new (func noninteractive proc stat &optional suc-msg)
  ...
  (let ((temp-buffer (process-buffer proc)))
    (unwind-protect
        (let ((header (twitter3-get-response-header temp-buffer))
              (mes nil))
          (if (string-match twitter3-http-status-line-regexp header)
          ...
          (when (and mes (twitter3-buffer-active-p))
            (message mes)))
      ;; unwindforms
      (twitter3-release-process proc)
      ...
      (let* ((request-str
        ...
        (lexical-let ((sentinel sentinel)
          ...
          (set-process-sentinel
           proc
           (lambda (&rest args)
             (apply #'twitter3-http-default-sentinel
    ...
    (when (string= "POST" method)
      ...
      (push (cons "Content-Type" "text/plain") headers))
                        (when twitter3-proxy-use
                          ...
                          (when (and twitter3-proxy-user twitter3-proxy-password)
                            (concat
                             "Proxy-Authorization: Basic "
                             (base64-encode-string
                              (concat twitter3-proxy-user ":"
...
(defun twitter3-http-default-sentinel (func noninteractive proc stat &optional suc-msg)
  (debug-printf "http-default-sentinel: proc=%s stat=%s" proc stat)
  (let ((temp-buffer (process-buffer proc)))
    (unwind-protect
        (let ((header (twitter3-get-response-header temp-buffer))
...
(defun twitter3-release-process (proc)
  (let ((spec (twitter3-get-timeline-spec-from-process proc)))
    (setq twitter3-process-info-alist
          (delete `(,proc ,spec) twitter3-process-info-alist))))

(defun twitter3-get-timeline-spec-from-process (proc)
  (let ((entry (assoc proc twitter3-process-info-alist)))
    (if entry
        (elt entry 1)
      nil)))
...
(defun twitter3-manage-favorites (method id)
  (twitter3-http-post-new "twitter.com"
