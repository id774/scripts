...
(defconst twitter3-mode-version "0.9.0")
...
(defconst twitter3-max-number-of-tweets-on-retrieval 200
  "The maximum number of `twitter3-number-of-tweets-on-retrieval'.")

(defvar twitter3-number-of-tweets-on-retrieval 20
  "*The number of tweets which will be retrieved in one request.
The upper limit is `twitter3-max-number-of-tweets-on-retrieval'.")

(defvar twitter3-tinyurl-service 'tinyurl
  "The service to use. One of 'tinyurl' or 'toly'")

(defvar twitter3-tinyurl-services-map
...
(defvar twitter3-tweet-history nil)
(defvar twitter3-user-history nil)
(defvar twitter3-timeline-history nil)
(defvar twitter3-hashtag-history nil)

(defvar twitter3-current-hashtag nil
  "A hash tag string currently set. You can set it by calling
`twitter3-set-current-hashtag'")

(defvar twitter3-timer nil
  "Timer object for timeline refreshing will be stored here.
DO NOT SET VALUE MANUALLY.")

(defvar twitter3-timer-interval 90
...
(defvar twitter3-username-active nil
  "Copy of `twitter3-username' for internal use.")
...
dangerous.")
(defvar twitter3-password-active nil
  "Copy of `twitter3-password' for internal use.")

(defvar twitter3-initial-timeline-spec-string ":friends"
  "The initial timeline spec string.")

(defvar twitter3-timeline-spec-alias nil
...
(defvar twitter3-last-requested-timeline-spec-string nil
  "The last requested timeline spec string.")
(defvar twitter3-last-retrieved-timeline-spec-string nil
  "The last successfully retrieved timeline spec string.")
(defvar twitter3-list-index-retrieved nil)
...
(defvar twitter3-status-format "%i %s: %t %r%R%p [%C]"
...
(defvar twitter3-retweet-format "RT: %t (via @%s)"
...
(defvar twitter3-use-show-minibuffer-length t
...
(defvar twitter3-notify-successful-http-get t)

(defvar twitter3-use-ssl t
...
(defvar twitter3-use-native-retweet nil
...
(defvar twitter3-proxy-keep-alive nil)
(defvar twitter3-proxy-server global-proxy-server
  "*The proxy server for `twitter3-mode'.
If nil, it is initialized on entering `twitter3-mode'.
The port number is specified by `twitter3-proxy-port'.")
(defvar twitter3-proxy-port global-proxy-port
  "*The port number of a proxy server for `twitter3-mode'.
If nil, it is initialized on entering `twitter3-mode'.
The server is specified by `twitter3-proxy-server'.")
(defvar twitter3-proxy-user global-proxy-user)
(defvar twitter3-proxy-password global-proxy-password)


(defun twitter3-find-proxy (scheme)
  "Find proxy server and its port for `twitter3-mode' and returns
a cons pair of them.
...
(defun twitter3-setup-proxy ()
  (unless (and twitter3-proxy-server twitter3-proxy-port)
    (let ((proxy-info (or (if twitter3-use-ssl
			      (twitter3-find-proxy "https"))
			  (twitter3-find-proxy "http"))))
      (when proxy-info
	(let ((host (car proxy-info))
	      (port (cdr proxy-info)))
	  (setq twitter3-proxy-server host)
	  (setq twitter3-proxy-port port)))))
  (when (and twitter3-proxy-use
	     (null twitter3-proxy-server)
	     (null twitter3-proxy-port))
    (message "Disabling proxy due to lack of configuration.")
    (setq twitter3-proxy-use nil)))

(defun twitter3-toggle-proxy ()
  (interactive)
  (setq twitter3-proxy-use
	(not twitter3-proxy-use))
  (twitter3-update-mode-line)
  (message (if twitter3-proxy-use "Use Proxy:on" "Use Proxy:off")))

;;;
;;; to show image files
;;;

(defvar twitter3-icon-mode nil
  "You MUST NOT CHANGE this variable directly.
You should change through function'twitter3-icon-mode'")
...
icon mode; otherwise, turn off icon mode."
  ...
  (setq twitter3-icon-mode
	(if (null arg)
	    (not twitter3-icon-mode)
	  (> (prefix-numeric-value arg) 0)))
  (twitter3-update-mode-line)
  (twitter3-render-timeline))

(defvar twitter3-image-data-table
  (make-hash-table :test 'equal))

(defvar twitter3-image-stack nil)
(defvar twitter3-image-type-cache nil)
(defvar twitter3-convert-program (executable-find "convert"))
(defvar twitter3-convert-fix-size 48)
(defvar twitter3-use-convert (not (null twitter3-convert-program))
  "*This variable makes a sense only if `twitter3-convert-fix-size'
...
(defun twitter3-image-type (image-url buffer)
  "Return the type of a given image based on the URL(IMAGE-URL)
and its contents(BUFFER)"
  (let ((type-cache (assoc image-url twitter3-image-type-cache))
	(case-fold-search t))
    (if type-cache
	(cdr type-cache)
      (let ((image-type
		 ...
		 (let ((file-output (buffer-string)))
	      ...
	      (t nil))))
	(add-to-list 'twitter3-image-type-cache `(,image-url . ,image-type))
	image-type))))
...
(defun twitter3-user-agent-default-function ()
  ...
  (format "Emacs/%d.%d twitter3-mode/%s"
...
(defun twitter3-sign-string-default-function ()
  ...
  (if twitter3-sign-simple-string
      (format " [%s]" twitter3-sign-simple-string)
    ""))
...
(defun twitter3-ucs-to-char (num)
...
(defun twitter3-remove-duplicates (list)
...
(defun twitter3-completing-read (prompt collection &optional predicate require-match initial-input hist def inherit-input-method)
...
as a list of a string on Emacs21."
  ...
  (let* ((collection (twitter3-remove-duplicates collection))
         (collection
...
(defun twitter3-timeline-spec-to-string (timeline-spec &optional shorten)
...
If SHORTEN is non-nil, the abbreviated expression will be used."
  (let ((type (car timeline-spec))
	...
	(concat ":filter/"
		...
		"/"
		(twitter3-timeline-spec-to-string spec))))
     ((eq type 'merge)
      (concat "("
	      (mapconcat 'twitter3-timeline-spec-to-string value "+" )
	      ")"))
     (t
      nil))))

(defun twitter3-extract-timeline-spec (str &optional unresolved-aliases)
  ...
  (cond
   ...
   ((string-match "^:\\([a-z_-]+\\)" str)
	...
	(if (string-match "^:filter/\\(.*?[^\\]\\)??/" str)
	    (let* ((escaped-regexp (or (match-string 1 str) ""))
		   ...
		   (following (substring str (match-end 0)))
		   (pair (twitter3-extract-timeline-spec
   ...
   ((string-match "^\\$\\([a-zA-Z0-9_-]+\\)" str)
    (let* ((name (match-string 1 str))
	   ...
	   (value (cdr-safe (assoc name twitter3-timeline-spec-alias))))
      (if (member name unresolved-aliases)
	  (error "Alias \"%s\" includes a recursive reference" name)
	(if value
	    (twitter3-extract-timeline-spec
	     (concat value rest)
...
(defun twitter3-string-to-timeline-spec (spec-str)
  "Convert STR into a timeline spec.
Return nil if STR is invalid as a timeline spec."
  (let ((result-pair (twitter3-extract-timeline-spec spec-str)))
    (if (and result-pair (string= "" (cdr result-pair)))
	(car result-pair)
      nil)))

(defun twitter3-timeline-spec-primary-p (spec)
...
(defun twitter3-equal-string-as-timeline (spec-str1 spec-str2)
  "Return non-nil if SPEC-STR1 equals SPEC-STR2 as a timeline spec."
  (if (and (stringp spec-str1) (stringp spec-str2))
      (let ((spec1 (twitter3-string-to-timeline-spec spec-str1))
	    (spec2 (twitter3-string-to-timeline-spec spec-str2)))
	(equal spec1 spec2))
    nil))

(defun twitter3-timeline-spec-to-host-method (spec)
  (if (twitter3-timeline-spec-primary-p spec)
      (let ((type (car spec))
	    (value (cdr spec)))
	(cond
	 ((eq type 'user)
...
(defun twitter3-host-method-to-timeline-spec (host method)
  (cond
...
(defun twitter3-add-timeline-history (&optional timeline-spec)
  (let* ((spec-string
	  (if timeline-spec
	      (twitter3-timeline-spec-to-string timeline-spec t)
	    twitter3-last-retrieved-timeline-spec-string)))
    (when spec-string
      (when (or (null twitter3-timeline-history)
		(not (string= spec-string (car twitter3-timeline-history))))
	(if (functionp 'add-to-history)
	    (add-to-history 'twitter3-timeline-history spec-string)
	  (setq twitter3-timeline-history
		(cons spec-string twitter3-timeline-history)))))))
...
(if twitter3-mode-map
    (let ((km twitter3-mode-map))
      ...
      (define-key km "\C-c\C-h" 'twitter3-set-current-hashtag)
      ...
      (define-key km "d" 'twitter3-direct-message)
      (define-key km "v" 'twitter3-other-user-timeline)
      (define-key km "V" 'twitter3-visit-timeline)
      (define-key km "L" 'twitter3-other-user-list-interactive)
      ...
      (define-key km "\C-i" 'twitter3-goto-next-thing)
      (define-key km "\M-\C-i" 'twitter3-goto-previous-thing)
      (define-key km [backtab] 'twitter3-goto-previous-thing)
      (define-key km [backspace] 'backward-char)
      (define-key km "G" 'end-of-buffer)
      (define-key km "H" 'beginning-of-buffer)
      (define-key km "i" 'twitter3-icon-mode)
      (define-key km "s" 'twitter3-scroll-mode)
      (define-key km "t" 'twitter3-toggle-proxy)
      (define-key km "\C-c\C-p" 'twitter3-toggle-proxy)
      (define-key km "\C-c\C-q" 'twitter3-suspend)
      nil))

(defun twitter3-keybind-message ()
  (let ((important-commands
	 '(("Timeline" . twitter3-friends-timeline)
	   ("Replies" . twitter3-replies-timeline)
	   ("Update status" . twitter3-update-status-interactive)
	   ("Next" . twitter3-goto-next-status)
	   ("Prev" . twitter3-goto-previous-status))))
...
;;  0.1 t
;;  '(lambda ()
;;     (when (equal (buffer-name (current-buffer)) twitter3-buffer)
;;       (message (twitter3-keybind-message)))))
...
;;   (add-to-list 'minor-mode-alist '(twitter3-jojo-mode " tw-jojo"))
  (setq twitter3-username-active twitter3-username)
  (setq twitter3-password-active twitter3-password)
  (when twitter3-use-convert
    (if (null twitter3-convert-program)
	(setq twitter3-use-convert nil)
      (with-temp-buffer
	(call-process twitter3-convert-program nil (current-buffer) nil
		      "-version")
	(goto-char (point-min))
	(if (null (search-forward-regexp "\\(Image\\|Graphics\\)Magick" nil t))
	    (setq twitter3-use-convert nil)))))
  (twitter3-setup-proxy)
  )
...
(defun twitter3-update-mode-line ()
  "Update mode line"
  (let ((enabled-options nil)
	(spec-string twitter3-last-retrieved-timeline-spec-string))
    (when twitter3-jojo-mode
      (push "jojo" enabled-options))
    (when twitter3-icon-mode
      (push "icon" enabled-options))
    (when twitter3-scroll-mode
      (push "scroll" enabled-options))
    (when twitter3-proxy-use
      (push "proxy" enabled-options))
    (when twitter3-use-ssl
      (push "ssl" enabled-options))
    (setq mode-name
	  (concat twitter3-mode-string
		  (if spec-string
		      (concat " " spec-string)
		    "")
		  (if enabled-options
...
(defun twitter3-find-curl-program ()
...
(defun twitter3-start-http-session (method headers host port path parameters &optional noninteractive sentinel)
  "
METHOD    : http method
...
"
  (block nil
    ...
    (unless (assoc "User-Agent" headers)
      (setq headers (cons `("User-Agent" . ,(twitter3-user-agent))
			  headers)))

    (let ((curl-program nil))
      (when twitter3-use-ssl
	(cond 
	 ((not (setq curl-program (twitter3-find-curl-program)))
	  (if (yes-or-no-p "HTTPS(SSL) is not available because 'cURL' does not exist. Use HTTP instead? ")
	      (progn (setq twitter3-use-ssl nil)
		     (twitter3-update-mode-line))
...
-----END CERTIFICATE-----"))
      (setq twitter3-cert-file file-name))))

(defun twitter3-start-http-ssl-session (curl-program method headers host port path parameters &optional noninteractive sentinel)
  ;; TODO: use curl
  (let* ((request (twitter3-make-http-request
		   method headers host port path parameters))
	  ...
	  `("--include" "--silent"
	    ,@(mapcan (lambda (pair)
			(list "-H"
		      ...
		      headers)
	    "--cacert"
	    ,(twitter3-ensure-ca-cert))))
    (when twitter3-proxy-use
      (nconc curl-args `("-x" ,(format "%s:%s" twitter3-proxy-server
					 twitter3-proxy-port)))
      (when (and twitter3-proxy-user
		 twitter3-proxy-password)
	(nconc curl-args `("-U" ,(format "%s:%s" twitter3-proxy-user
					   twitter3-proxy-password)))))
...
;; TODO: proxy
(defun twitter3-start-http-non-ssl-session (method headers host port path parameters &optional noninteractive sentinel)
  (let ((request (twitter3-make-http-request
		  method headers host port path parameters)))
    (flet ((request (key) (funcall request key)))
      (let* ((request-str
	     ...
	     (server (if twitter3-proxy-use
			 twitter3-proxy-server
		       (request :host)))
	     (port (if twitter3-proxy-use
		       twitter3-proxy-port
		     (request :port)))
...
;;; TODO: proxy
(defun twitter3-make-http-request (method headers host port path parameters)
...
Available keywords:
  ...
  :query-string
  "
  (let* ((schema (if twitter3-use-ssl "https" "http"))
	 (default-port (if twitter3-use-ssl 443 80))
	 ...
	 (query-string
	  (mapconcat (lambda (pair)
		       (format
			"%s=%s"
			(twitter3-percent-encode (car pair))
			(twitter3-percent-encode (cdr pair))))
		     parameters
		     "&"))
	 )
...
(defun twitter3-http-application-headers (&optional method headers)
  "Retuns an assoc list of HTTP headers for twitter3-mode."
  (unless method
    (setq method "GET"))

  (let ((headers headers))
    (push (cons "User-Agent" (twitter3-user-agent)) headers)
    (push (cons "Authorization"
		(concat "Basic "
			(base64-encode-string
			 (concat
			  (twitter3-get-username)
			  ":"
    ...
    (when twitter3-proxy-use
      (when twitter3-proxy-keep-alive
...
(defun twitter3-http-get (host method &optional noninteractive parameters sentinel)
  (if (null sentinel)
      (setq sentinel 'twitter3-http-get-default-sentinel))

  (twitter3-start-http-session
   "GET" (twitter3-http-application-headers "GET")
   host nil (concat "/" method ".xml") parameters noninteractive sentinel))

(defun twitter3-created-at-to-seconds (created-at)
  (let ((encoded-time (apply 'encode-time (parse-time-string created-at))))
    (+ (* (car encoded-time) 65536)
       (cadr encoded-time))))

(defun twitter3-http-get-default-sentinel (temp-buffer noninteractive proc stat &optional suc-msg)
  (debug-printf "get-default-sentinel: proc=%s stat=%s" proc stat)
  (unwind-protect
      (let ((header (twitter3-get-response-header temp-buffer))
	    (body (twitter3-get-response-body temp-buffer))
	    (status nil))
	(if (string-match "HTTP/1\.[01] \\([a-zA-Z0-9 ]+\\)\r?\n" header)
		...
		(setq twitter3-new-tweets-count
		      (count t (mapcar
				...
				(reverse (twitter3-xmltree-to-status
					  body)))))
		(setq twitter3-timeline-data
		      (sort twitter3-timeline-data
			    (lambda (status1 status2)
			      (let ((created-at1
				     (twitter3-created-at-to-seconds
		...
		(setq twitter3-last-retrieved-timeline-spec-string
		      twitter3-last-requested-timeline-spec-string)
		(twitter3-render-timeline)
		(twitter3-add-timeline-history)
		(when twitter3-notify-successful-http-get
		  ))
	       (t (message status))))
	  (message "Failure: Bad http response.")))
    ;; unwindforms
    (when (and (not twitter3-debug-mode) (buffer-live-p temp-buffer))
      (kill-buffer temp-buffer)))
  )

;; XXX: this is a preliminary implementation because we should parse
;; xmltree in the function.
(defun twitter3-http-get-list-index-sentinel (temp-buffer noninteractive proc stat &optional suc-msg)
  (debug-printf "get-list-index-sentinel: proc=%s stat=%s" proc stat)
  (unwind-protect
      (let ((header (twitter3-get-response-header temp-buffer)))
	(if (not (string-match "HTTP/1\.[01] \\([a-zA-Z0-9 ]+\\)\r?\n" header))
	    (setq twitter3-list-index-retrieved "Failure: Bad http response.")
	  (let ((status (match-string-no-properties 1 header))
		(indexes nil))
	    (if (not (string-match "\r?\nLast-Modified: " header))
		(setq twitter3-list-index-retrieved
		...
		(with-current-buffer temp-buffer
		  (save-excursion
		    ...
		    (if indexes
			(setq twitter3-list-index-retrieved indexes)
		      (setq twitter3-list-index-retrieved "")))))
	       (t
		(setq twitter3-list-index-retrieved status)))))))
    ;; unwindforms
    (when (and (not twitter3-debug-mode) (buffer-live-p temp-buffer))
      (kill-buffer temp-buffer)))
  )

(defun twitter3-http-post (host method &optional parameters contents sentinel)
...
PARAMETERS is alist of URI parameters.
 ex) ((\"mode\" . \"view\") (\"page\" . \"6\")) => <URI>?mode=view&page=6"
  (if (null sentinel)
      (setq sentinel 'twitter3-http-post-default-sentinel))

  (twitter3-start-http-session
   "POST" (twitter3-http-application-headers "POST")
   host nil (concat "/" method ".xml") parameters noninteractive sentinel))

(defun twitter3-http-post-default-sentinel (temp-buffer noninteractive proc stat &optional suc-msg)
  (debug-printf "post-default-sentinel: proc=%s stat=%s" proc stat)
  (unwind-protect
      (let ((header (twitter3-get-response-header temp-buffer))
	    ;; (body (twitter3-get-response-body temp-buffer)) not used now.
	    (status nil))
	(if (string-match "HTTP/1\.[01] \\([a-zA-Z0-9 ]+\\)\r?\n" header)
	    (setq status (match-string-no-properties 1 header))
	  (setq status
...
(defun twitter3-get-response-header (buffer)
...
(defun twitter3-get-response-body (buffer)
...
(defun twitter3-status-to-status-datum (status)
  (flet ((assq-get (item seq)
    ...
    (let* ((status-data (cddr status))
      ...
      (when (and retweeted-status-data twitter3-use-native-retweet)
	(setq original-user-screen-name (twitter3-decode-html-entities
					 (assq-get 'screen_name user-data))
	      original-user-name (twitter3-decode-html-entities
	...
	(when regex-index
      ...
      (when (or (null twitter3-timeline-last-update)
                (< (twitter3-created-at-to-seconds
                    twitter3-timeline-last-update)
                   (twitter3-created-at-to-seconds created-at)))
...
(defun twitter3-make-display-spec-for-icon (image-url)
...
image are displayed."
  (let* ((image-data (twitter3-retrieve-image image-url))
	 (image-spec
	  `(image :type ,(car image-data)
		  :data ,(cdr image-data))))
    (if (not (image-type-available-p (car image-data)))
	nil
      (if (and twitter3-convert-fix-size (not twitter3-use-convert))
	  (let* ((size (if (cdr image-data)
			   (image-size image-spec t)
			 '(48 . 48)))
		 (width (car size))
		 (height (cdr size))
		 (fixed-length twitter3-convert-fix-size)
...
(defun twitter3-format-string (string prefix replacement-table)
...
variable `twitter3-status-format'"
  (flet ((attr (key)
	       (assocref key status))
	 (profile-image
	  ()
	  (let ((profile-image-url (attr 'user-profile-image-url))
		(icon-string "\n  "))
	    (unless (gethash
		     `(,profile-image-url . ,twitter3-convert-fix-size)
		     twitter3-image-data-table)
	      (add-to-list 'twitter3-image-stack profile-image-url))
	    
	    (when (and icon-string twitter3-icon-mode)
	      (let ((display-spec
		     (twitter3-make-display-spec-for-icon profile-image-url)))
		(when display-spec
	 ...
	 (make-string-with-url-property
	  ...
	  (let ((result (copy-sequence str)))
	    (add-text-properties
	     0 (length result)
	     `(mouse-face highlight face twitter3-uri-face uri ,url)
	     result)
	    result)))
    (let* ((replace-table
	    `(("%" . "%")
	       ...
	       ,(let* ((created-at
			...
			(cond
			 ...
			 (t (format-time-string "%I:%M %p %B %d, %Y"
						created-at))))
		       (url
			(twitter3-get-status-url (attr 'user-screen-name)
						   (attr 'id))))
		 ...
		 (let ((str (cdr (assq 'following-string context)))
		  ...
		  (if (or (string= "" reply-id) (string= "" reply-name))
		    ...
		    (let ((in-reply-to-string
			  ...
			  (url
			   (twitter3-get-status-url reply-name reply-id)))
		      (concat " "
			      (make-string-with-url-property
		  ...
		  (let* ((str (cdr (assq 'processed-string context)))
			 ...
			 (table (cdr (assq 'replacement-table context))))
		    (store-match-data match-data)
		    (let* ((postfix (twitter3-format-string
	   ...
	   (formatted-status
	    (twitter3-format-string format-str "%" replace-table)))
      (add-text-properties
...
(defun twitter3-show-minibuffer-length (&optional beg end len)
  ...
  (when (minibuffer-window-active-p (selected-window))
    ...
    (let* ((deactivate-mark deactivate-mark)
	   ...
	   (sign-len (length (twitter3-sign-string)))
	   (mes (if (< 0 sign-len)
		    (format "%d=%d+%d"
...
(defun twitter3-setup-minibuffer ()
  (add-hook 'post-command-hook 'twitter3-show-minibuffer-length t t))

(defun twitter3-finish-minibuffer ()
  (remove-hook 'post-command-hook 'twitter3-show-minibuffer-length t))

(defun twitter3-status-not-blank-p (status)
  (with-temp-buffer
    (insert status)
    (goto-char (point-min))
    ;; skip user name
...
(defun twitter3-update-status-from-minibuffer (&optional init-str reply-to-id)
  (when (and (null init-str)
	     twitter3-current-hashtag)
    (setq init-str (format " #%s " twitter3-current-hashtag)))
  (let ((status init-str)
	(sign-str (twitter3-sign-string))
	(not-posted-p t)
	(prompt "status: ")
	(map minibuffer-local-map)
	(minibuffer-message-timeout nil))
    (define-key map (kbd "<f4>") 'twitter3-tinyurl-replace-at-point)
    (when twitter3-use-show-minibuffer-length
      (add-hook 'minibuffer-setup-hook 'twitter3-setup-minibuffer t)
      (add-hook 'minibuffer-exit-hook 'twitter3-finish-minibuffer t))
    (unwind-protect
	(while not-posted-p
	  (setq status (read-from-minibuffer prompt status map nil 'twitter3-tweet-history nil t))
	  (let ((status-with-sign (concat status sign-str)))
	    (if (< 140 (length status-with-sign))
		(setq prompt "status (too long): ")
	      (progn
		(setq prompt "status: ")
		(when (twitter3-status-not-blank-p status)
		  (let ((parameters `(("status" . ,status-with-sign)
				      ("source" . "twmode")
				      ,@(if reply-to-id
					    `(("in_reply_to_status_id"
					       . ,reply-to-id))))))
		    (twitter3-http-post "twitter.com" "statuses/update" parameters)
		    (setq not-posted-p nil)))
		))))
      ;; unwindforms
      (when (memq 'twitter3-setup-minibuffer minibuffer-setup-hook)
	(remove-hook 'minibuffer-setup-hook 'twitter3-setup-minibuffer))
      (when (memq 'twitter3-finish-minibuffer minibuffer-exit-hook)
	(remove-hook 'minibuffer-exit-hook 'twitter3-finish-minibuffer))
      )))

(defun twitter3-get-list-index (username)
  (twitter3-http-get "api.twitter.com"
		       (concat "1/" username "/lists")
		       t nil
		       'twitter3-http-get-list-index-sentinel))

(defun twitter3-get-list-index-sync (username)
  (setq twitter3-list-index-retrieved nil)
  (twitter3-get-list-index username)
  (while (not twitter3-list-index-retrieved)
    (sit-for 0.1))
  (cond
   ((stringp twitter3-list-index-retrieved)
    (if (string= "" twitter3-list-index-retrieved)
	(message (concat username " has no list"))
      (message twitter3-list-index-retrieved))
    nil)
   ((listp twitter3-list-index-retrieved)
    twitter3-list-index-retrieved)))

(defun twitter3-manage-friendships (method username)
  (twitter3-http-post "twitter.com"
			(concat "friendships/" method)
			`(("screen_name" . ,username)
			  ("source" . "twmode"))))

(defun twitter3-manage-favorites (method id)
  (twitter3-http-post "twitter.com"
			(concat "favorites/" method "/" id)
			`(("source" . "twmode"))))

(defun twitter3-get-twits (host method &optional noninteractive id)
  (let ((buf (get-buffer twitter3-buffer)))
    (if (not buf)
	(twitter3-stop)
      (let* ((default-count 20)
	     (count twitter3-number-of-tweets-on-retrieval)
	     (count (cond
	     ...
	     (count (min (max 1 count)
			 twitter3-max-number-of-tweets-on-retrieval))
	...
	(if id
	  ...
	  (when twitter3-timeline-last-update
	...
	(twitter3-http-get host method
			     noninteractive parameters))))

  (if (and twitter3-icon-mode window-system
	   twitter3-image-stack)
      (mapc 'twitter3-retrieve-image twitter3-image-stack)
    ))

(defun twitter3-get-and-render-timeline (spec &optional noninteractive id)
  (let* ((original-spec spec)
	 (spec-string (if (stringp spec)
			  spec
			(twitter3-timeline-spec-to-string spec)))
	 (spec ;; normalized spec.
	  (twitter3-string-to-timeline-spec spec-string)))
    (when (null spec)
      (error "\"%s\" is invalid as a timeline spec"
	     (or spec-string original-spec)))
    (setq twitter3-last-requested-timeline-spec-string spec-string)
    (unless
	(and twitter3-last-retrieved-timeline-spec-string
	     (twitter3-equal-string-as-timeline
	      spec-string twitter3-last-retrieved-timeline-spec-string))
      (setq twitter3-timeline-last-update nil
	    twitter3-timeline-data nil))
    (if (twitter3-timeline-spec-primary-p spec)
	(let ((pair (twitter3-timeline-spec-to-host-method spec)))
	  (when pair
	    (let ((host (car pair))
		  (method (cadr pair)))
	      (twitter3-get-twits host method noninteractive id))))
      (let ((type (car spec)))
	(error "%s has not been supported yet" type)))))

(defun twitter3-retrieve-image (image-url)
  (let ((image-data (gethash `(,image-url . ,twitter3-convert-fix-size)
			     twitter3-image-data-table)))
    (when (not image-data)
      (let ((image-type nil)
	    (image-spec nil)
	    (converted-image-size
	     `(,twitter3-convert-fix-size . ,twitter3-convert-fix-size)))
	(with-temp-buffer
	  ...
	  (let ((coding-system-for-read 'binary)
	    ...
	    (setq image-type (twitter3-image-type image-url
						    (current-buffer)))
	    (setq image-spec `(image :type ,image-type
				     :data ,(buffer-string)))
	    (when (and twitter3-convert-fix-size twitter3-use-convert
		       (not
	      ...
	      (call-process-region 
	       (point-min) (point-max)
	       twitter3-convert-program
	       t t nil
	       (if image-type (format "%s:-" image-type) "-")
	       "-resize"
	       (format "%dx%d" twitter3-convert-fix-size
		       twitter3-convert-fix-size)
	       "xpm:-")
	      (setq image-type 'xpm))
	    (setq image-data `(,image-type . ,(buffer-string))))
	  (puthash `(,image-url . ,twitter3-convert-fix-size)
		   image-data
		   twitter3-image-data-table))))
    image-data))

(defun twitter3-tinyurl-get (longurl)
  "Tinyfy LONGURL"
  (let ((api (cdr (assoc twitter3-tinyurl-service
			 twitter3-tinyurl-services-map))))
    (unless api
      (error "Invaild `twitter3-tinyurl-service'. try one of %s"
...
(defun twitter3-friends-timeline ()
  (interactive)
  (twitter3-get-and-render-timeline '(friends)))

(defun twitter3-replies-timeline ()
  (interactive)
  (twitter3-get-and-render-timeline '(replies)))

(defun twitter3-public-timeline ()
  (interactive)
  (twitter3-get-and-render-timeline '(public)))

(defun twitter3-user-timeline ()
  (interactive)
  (twitter3-get-and-render-timeline `(user ,(twitter3-get-username))))

(defun twitter3-current-timeline-noninteractive ()
  (twitter3-current-timeline t))

(defun twitter3-current-timeline (&optional noninteractive)
  (interactive)
  (let ((spec (or twitter3-last-retrieved-timeline-spec-string
		  twitter3-initial-timeline-spec-string)))
    (twitter3-get-and-render-timeline spec noninteractive)))
...
(defun twitter3-set-current-hashtag (&optional tag)
  (interactive)
  (unless tag
    (setq tag (twitter3-completing-read "hashtag (blank to clear): #"
					  twitter3-hashtag-history
					  nil nil
					  twitter3-current-hashtag
					  'twitter3-hashtag-history))
    (message
     (if (eq 0 (length tag))
	 (progn (setq twitter3-current-hashtag nil)
		"Current hashtag is not set.")
       (progn
	 (setq twitter3-current-hashtag tag)
	 (format "Current hashtag is #%s" twitter3-current-hashtag))))))

(defun twitter3-erase-old-statuses ()
  (interactive)
  (setq twitter3-timeline-data nil)
  (if (not twitter3-last-retrieved-timeline-spec-string)
      (setq twitter3-last-retrieved-timeline-spec-string
	    twitter3-initial-timeline-spec-string)
    (let* ((spec-string twitter3-last-retrieved-timeline-spec-string)
	   (spec (twitter3-string-to-timeline-spec spec-string))
	   (pair (twitter3-timeline-spec-to-host-method spec))
	   (host (car pair))
	   (method (cadr pair)))
	...
	(let* ((system-time-locale "C")
		...
		(twitter3-global-strftime
		 "%a, %d %b %Y %H:%M:%S GMT"
		 twitter3-timeline-last-update)))
	  (twitter3-http-get host method nil `(("since" . ,since))))))))
...
(defun twitter3-tinyurl-replace-at-point ()
  "Replace the url at point with a tiny version."
  (interactive)
  (let ((url-bounds (bounds-of-thing-at-point 'url)))
    (when url-bounds
      (let ((url (twitter3-tinyurl-get (thing-at-point 'url))))
	(when url
	  (save-restriction
...
(defun twitter3-retweet ()
  (interactive)
  (if twitter3-use-native-retweet
      (twitter3-native-retweet)
    (twitter3-organic-retweet)))

(defun twitter3-organic-retweet ()
  (interactive)
  (let ((username (get-text-property (point) 'username))
	(text (get-text-property (point) 'text))
	(id (get-text-property (point) 'id))
	(retweet-time (current-time))
	(format-str (or twitter3-retweet-format
			"RT: %t (via @%s)")))
    (when username
      (let ((prefix "%")
	    (replace-table
	...
	(twitter3-update-status-from-minibuffer
	 (twitter3-format-string format-str prefix replace-table))
	))))
...
(defun twitter3-follow (&optional remove)
  (interactive)
  (let ((username (get-text-property (point) 'username))
	(method (if remove "destroy" "create"))
	(mes (if remove "unfollowing" "following")))
    (unless username
      (setq username (twitter3-read-username-with-completion
		      "who: " "" 'twitter3-user-history)))
    (if (> (length username) 0)
	(when (y-or-n-p (format "%s %s? " mes username))
	  (twitter3-manage-friendships method username))
      (message "No user selected"))))

(defun twitter3-unfollow ()
  (interactive)
  (twitter3-follow t))

(defun twitter3-native-retweet ()
  (interactive)
  (let ((id (get-text-property (point) 'id))
	...
	(let ((mes (format "Retweet \"%s\"? "
			   (if (> (length text) len)
			     ...
			     text))))
	  (when (y-or-n-p mes)
	    (twitter3-http-post "api.twitter.com"
			(concat "1/statuses/retweet/" id)
			`(("source" . "twmode")))))
      (message "No status selected"))))

(defun twitter3-favorite (&optional remove)
  (interactive)
  (let ((id (get-text-property (point) 'id))
	(text (get-text-property (point) 'text))
	(len 25) ;; XXX
	...
	(let ((mes (format "%s \"%s\"? "
			   ...
			   (if (> (length text) len)
			     ...
			     text))))
	  (when (y-or-n-p mes)
	    (twitter3-manage-favorites method id)))
      (message "No status selected"))))

(defun twitter3-unfavorite ()
  (interactive)
  (twitter3-favorite t))

(defun twitter3-visit-timeline (&optional timeline-spec initial)
  (interactive)
  (let ((timeline-spec
	 (or timeline-spec
	     (twitter3-read-timeline-spec-with-completion
	      "timeline: " initial t))))
    (when timeline-spec
      (twitter3-get-and-render-timeline timeline-spec))))
...
(defun twitter3-other-user-timeline ()
  ...
  (let* ((username (get-text-property (point) 'username))
	 ...
	 (spec (cond (screen-name-in-text `(user ,screen-name-in-text))
		     ...
		     (t nil))))
    (if spec
	(twitter3-get-and-render-timeline spec)
      (message "No user selected"))))

(defun twitter3-other-user-timeline-interactive ()
  (interactive)
  (let ((username
	 (twitter3-read-username-with-completion
	  "user: " nil
	  'twitter3-user-history)))
    (if (> (length username) 0)
	(twitter3-get-and-render-timeline `(user ,username))
      (message "No user selected"))))

(defun twitter3-other-user-list-interactive ()
  (interactive)
  (let ((username (twitter3-read-username-with-completion
		   "whose list: "
		   (get-text-property (point) 'username)
		   'twitter3-user-history)))
    (if (string= "" username)
	(message "No user selected")
      (let* ((list-name (twitter3-read-list-name username))
	     (spec `(list ,username ,list-name)))
	(when list-name
	  (twitter3-get-and-render-timeline spec))))))

(defun twitter3-direct-message ()
  (interactive)
  (let ((username (get-text-property (point) 'username)))
    (if username
	(twitter3-update-status-from-minibuffer (concat "d " username " ")))))
...
(defun twitter3-make-list-from-assoc (key data)
  (mapcar (lambda (status)
	    (cdr (assoc key status)))
	  data))

(defun twitter3-read-username-with-completion (prompt init-user &optional history)
  (let ((collection
	 (append (twitter3-make-list-from-assoc
		  'user-screen-name twitter3-timeline-data)
		 twitter3-user-history)))
    (twitter3-completing-read prompt collection nil nil init-user history)))

(defun twitter3-read-list-name (username &optional list-index)
  (let* ((list-index (or list-index
			 (twitter3-get-list-index-sync username)))
	 (prompt (concat username "'s list: "))
	 (listname
	  (if list-index
	      (twitter3-completing-read prompt list-index nil t nil)
	    nil)))
    (if (string= "" listname)
	nil
      listname)))

(defun twitter3-read-timeline-spec-with-completion (prompt initial &optional as-string)
  (let* ((dummy-hist (append twitter3-timeline-history
			     (twitter3-make-list-from-assoc
			      'user-screen-name twitter3-timeline-data)))
	 (spec-string (twitter3-completing-read prompt dummy-hist
						  nil nil initial 'dummy-hist))
	 (spec-string
	  (if (string-match "^\\([^/]+\\)/$" spec-string)
	      (let* ((username (match-string 1 spec-string))
		     (list-index (twitter3-get-list-index-sync username))
		     (listname
		      (if list-index
			  (twitter3-read-list-name username list-index)
			nil)))
		(if listname
...
(defun twitter3-get-username ()
  (or twitter3-username-active
      (setq twitter3-username-active (read-string "your twitter username: "))))

(defun twitter3-get-password ()
  (or twitter3-password-active
      (setq twitter3-password-active (read-passwd "your twitter password: "))))
...
(defun twitter3-goto-next-status ()
  ...
  (let ((pos))
	...
	(goto-char pos)
      (let ((id (get-text-property (point) 'id)))
        (if id
	    (twitter3-get-and-render-timeline
	     twitter3-last-retrieved-timeline-spec-string
	     nil id))))))
...
(defun twitter3-goto-previous-status ()
  ...
  (let* ((current-pos (point))
         (prev-pos (twitter3-get-previous-username-face-pos current-pos)))
  ...
  (let ((prop))
	...
	(when (eq pos nil)
	  (let ((head-prop (get-text-property (point-min) 'face)))
	    (if (and
		 (not (eq prop twitter3-username-face))
		 (eq head-prop twitter3-username-face))
		(setq pos (point-min))
	      (throw 'not-found nil)
	      )))
	(setq prop (get-text-property pos 'face)))
...
(defun twitter3-goto-previous-status-of-user ()
  ...
  (let ((user-name (twitter3-get-username-at-pos (point)))
    ...
    (if (and pos
             (not (eq pos prev-pos))
             (equal (twitter3-get-username-at-pos pos) user-name))
	(goto-char pos)
      (if user-name
	  (message "Start of %s's status." user-name)
	(message "Invalid user-name.")))))

(defun twitter3-goto-next-thing (&optional backword)
			...
			(face-pred
			 (lambda (face)
			   (cond
			    ...
			    (t nil)))))
		   (member-if face-pred
			      '(twitter3-username-face
				twitter3-uri-face)))))
      (setq pos (funcall propety-change-f pos 'face)))
    (when pos
      (goto-char pos))))

(defun twitter3-goto-previous-thing (&optional backword)
  "Go to previous interesting thing. ex) username, URI, ... "
  (interactive)
  (twitter3-goto-next-thing (not backword)))
...
(defun twitter3-suspend ()
  "Suspend twitter3-mode then switch to another buffer."
  (interactive)
