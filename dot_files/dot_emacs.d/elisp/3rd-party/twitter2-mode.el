...
(defconst twitter2-mode-version "0.9.0")
...
(defconst twitter2-max-number-of-tweets-on-retrieval 200
  "The maximum number of `twitter2-number-of-tweets-on-retrieval'.")

(defvar twitter2-number-of-tweets-on-retrieval 20
  "*The number of tweets which will be retrieved in one request.
The upper limit is `twitter2-max-number-of-tweets-on-retrieval'.")

(defvar twitter2-tinyurl-service 'tinyurl
  "The service to use. One of 'tinyurl' or 'toly'")

(defvar twitter2-tinyurl-services-map
...
(defvar twitter2-tweet-history nil)
(defvar twitter2-user-history nil)
(defvar twitter2-timeline-history nil)
(defvar twitter2-hashtag-history nil)

(defvar twitter2-current-hashtag nil
  "A hash tag string currently set. You can set it by calling
`twitter2-set-current-hashtag'")

(defvar twitter2-timer nil
  "Timer object for timeline refreshing will be stored here.
DO NOT SET VALUE MANUALLY.")

(defvar twitter2-timer-interval 90
...
(defvar twitter2-username-active nil
  "Copy of `twitter2-username' for internal use.")
...
dangerous.")
(defvar twitter2-password-active nil
  "Copy of `twitter2-password' for internal use.")

(defvar twitter2-initial-timeline-spec-string ":friends"
  "The initial timeline spec string.")

(defvar twitter2-timeline-spec-alias nil
...
(defvar twitter2-last-requested-timeline-spec-string nil
  "The last requested timeline spec string.")
(defvar twitter2-last-retrieved-timeline-spec-string nil
  "The last successfully retrieved timeline spec string.")
(defvar twitter2-list-index-retrieved nil)

(defvar twitter2-new-tweets-count 0
  "Number of new tweets when `twitter2-new-tweets-hook' is run")

(defvar twitter2-new-tweets-hook nil
  "Hook run when new twits are received.

You can read `twitter2-new-tweets-count' to get the number of new
...
(defvar twitter2-status-format "%i %s: %t %r%R%p [%C]"
...
(defvar twitter2-retweet-format "RT: %t (via @%s)"
...
(defvar twitter2-use-show-minibuffer-length t
...
(defvar twitter2-notify-successful-http-get t)

(defvar twitter2-use-ssl t
...
(defvar twitter2-use-native-retweet nil
...
(defvar twitter2-proxy-keep-alive nil)
(defvar twitter2-proxy-server global-proxy-server
  "*The proxy server for `twitter2-mode'.
If nil, it is initialized on entering `twitter2-mode'.
The port number is specified by `twitter2-proxy-port'.")
(defvar twitter2-proxy-port global-proxy-port
  "*The port number of a proxy server for `twitter2-mode'.
If nil, it is initialized on entering `twitter2-mode'.
The server is specified by `twitter2-proxy-server'.")
(defvar twitter2-proxy-user global-proxy-user)
(defvar twitter2-proxy-password global-proxy-password)


(defun twitter2-find-proxy (scheme)
  "Find proxy server and its port for `twitter2-mode' and returns
a cons pair of them.
...
(defun twitter2-setup-proxy ()
  (unless (and twitter2-proxy-server twitter2-proxy-port)
    (let ((proxy-info (or (if twitter2-use-ssl
			      (twitter2-find-proxy "https"))
			  (twitter2-find-proxy "http"))))
      (when proxy-info
	(let ((host (car proxy-info))
	      (port (cdr proxy-info)))
	  (setq twitter2-proxy-server host)
	  (setq twitter2-proxy-port port)))))
  (when (and twitter2-proxy-use
	     (null twitter2-proxy-server)
	     (null twitter2-proxy-port))
    (message "Disabling proxy due to lack of configuration.")
    (setq twitter2-proxy-use nil)))

(defun twitter2-toggle-proxy ()
  (interactive)
  (setq twitter2-proxy-use
	(not twitter2-proxy-use))
  (twitter2-update-mode-line)
  (message (if twitter2-proxy-use "Use Proxy:on" "Use Proxy:off")))

;;;
;;; to show image files
;;;

(defvar twitter2-icon-mode nil
  "You MUST NOT CHANGE this variable directly.
You should change through function'twitter2-icon-mode'")
...
icon mode; otherwise, turn off icon mode."
  ...
  (setq twitter2-icon-mode
	(if (null arg)
	    (not twitter2-icon-mode)
	  (> (prefix-numeric-value arg) 0)))
  (twitter2-update-mode-line)
  (twitter2-render-timeline))

(defvar twitter2-image-data-table
  (make-hash-table :test 'equal))

(defvar twitter2-image-stack nil)
(defvar twitter2-image-type-cache nil)
(defvar twitter2-convert-program (executable-find "convert"))
(defvar twitter2-convert-fix-size 48)
(defvar twitter2-use-convert (not (null twitter2-convert-program))
  "*This variable makes a sense only if `twitter2-convert-fix-size'
...
(defun twitter2-image-type (image-url buffer)
  "Return the type of a given image based on the URL(IMAGE-URL)
and its contents(BUFFER)"
  (let ((type-cache (assoc image-url twitter2-image-type-cache))
	(case-fold-search t))
    (if type-cache
	(cdr type-cache)
      (let ((image-type
		 ...
		 (let ((file-output (buffer-string)))
	      ...
	      (t nil))))
	(add-to-list 'twitter2-image-type-cache `(,image-url . ,image-type))
	image-type))))
...
(defun twitter2-user-agent-default-function ()
  ...
  (format "Emacs/%d.%d twitter2-mode/%s"
	  emacs-major-version emacs-minor-version
	  twitter2-mode-version))

(defvar twitter2-sign-simple-string nil)

(defun twitter2-sign-string-default-function ()
  "Tweet append sign string:simple "
  (if twitter2-sign-simple-string
      (format " [%s]" twitter2-sign-simple-string)
    ""))

(defvar twitter2-user-agent-function 'twitter2-user-agent-default-function)
(defvar twitter2-sign-string-function 'twitter2-sign-string-default-function)

(defun twitter2-user-agent ()
  "Return User-Agent header string."
  (funcall twitter2-user-agent-function))

(defun twitter2-sign-string ()
  "Return Tweet sign string."
  (funcall twitter2-sign-string-function))
...
(defun twitter2-ucs-to-char (num)
...
(defun twitter2-remove-duplicates (list)
...
(defun twitter2-completing-read (prompt collection &optional predicate require-match initial-input hist def inherit-input-method)
...
as a list of a string on Emacs21."
  ...
  (let* ((collection (twitter2-remove-duplicates collection))
         (collection
...
(defun twitter2-timeline-spec-to-string (timeline-spec &optional shorten)
...
If SHORTEN is non-nil, the abbreviated expression will be used."
  (let ((type (car timeline-spec))
	...
	(concat ":filter/"
		...
		"/"
		(twitter2-timeline-spec-to-string spec))))
     ((eq type 'merge)
      (concat "("
	      (mapconcat 'twitter2-timeline-spec-to-string value "+" )
	      ")"))
     (t
      nil))))

(defun twitter2-extract-timeline-spec (str &optional unresolved-aliases)
  ...
  (cond
   ...
   ((string-match "^:\\([a-z_-]+\\)" str)
	...
	(if (string-match "^:filter/\\(.*?[^\\]\\)??/" str)
	    (let* ((escaped-regexp (or (match-string 1 str) ""))
		   ...
		   (following (substring str (match-end 0)))
		   (pair (twitter2-extract-timeline-spec
   ...
   ((string-match "^\\$\\([a-zA-Z0-9_-]+\\)" str)
    (let* ((name (match-string 1 str))
	   ...
	   (value (cdr-safe (assoc name twitter2-timeline-spec-alias))))
      (if (member name unresolved-aliases)
	  (error "Alias \"%s\" includes a recursive reference" name)
	(if value
	    (twitter2-extract-timeline-spec
	     (concat value rest)
...
(defun twitter2-string-to-timeline-spec (spec-str)
  "Convert STR into a timeline spec.
Return nil if STR is invalid as a timeline spec."
  (let ((result-pair (twitter2-extract-timeline-spec spec-str)))
    (if (and result-pair (string= "" (cdr result-pair)))
	(car result-pair)
      nil)))

(defun twitter2-timeline-spec-primary-p (spec)
...
(defun twitter2-equal-string-as-timeline (spec-str1 spec-str2)
  "Return non-nil if SPEC-STR1 equals SPEC-STR2 as a timeline spec."
  (if (and (stringp spec-str1) (stringp spec-str2))
      (let ((spec1 (twitter2-string-to-timeline-spec spec-str1))
	    (spec2 (twitter2-string-to-timeline-spec spec-str2)))
	(equal spec1 spec2))
    nil))

(defun twitter2-timeline-spec-to-host-method (spec)
  (if (twitter2-timeline-spec-primary-p spec)
      (let ((type (car spec))
	    (value (cdr spec)))
	(cond
	 ((eq type 'user)
...
(defun twitter2-host-method-to-timeline-spec (host method)
  (cond
...
(defun twitter2-add-timeline-history (&optional timeline-spec)
  (let* ((spec-string
	  (if timeline-spec
	      (twitter2-timeline-spec-to-string timeline-spec t)
	    twitter2-last-retrieved-timeline-spec-string)))
    (when spec-string
      (when (or (null twitter2-timeline-history)
		(not (string= spec-string (car twitter2-timeline-history))))
	(if (functionp 'add-to-history)
	    (add-to-history 'twitter2-timeline-history spec-string)
	  (setq twitter2-timeline-history
		(cons spec-string twitter2-timeline-history)))))))
...
(if twitter2-mode-map
    (let ((km twitter2-mode-map))
      ...
      (define-key km "\C-c\C-d" 'twitter2-erase-old-statuses)
      (define-key km "\C-c\C-m" 'twitter2-retweet)
      (define-key km "\C-c\C-h" 'twitter2-set-current-hashtag)
      ...
      (define-key km "d" 'twitter2-direct-message)
      (define-key km "v" 'twitter2-other-user-timeline)
      (define-key km "V" 'twitter2-visit-timeline)
      (define-key km "L" 'twitter2-other-user-list-interactive)
      ...
      (define-key km "\C-i" 'twitter2-goto-next-thing)
      (define-key km "\M-\C-i" 'twitter2-goto-previous-thing)
      (define-key km [backtab] 'twitter2-goto-previous-thing)
      (define-key km [backspace] 'backward-char)
      (define-key km "G" 'end-of-buffer)
      (define-key km "H" 'beginning-of-buffer)
      (define-key km "i" 'twitter2-icon-mode)
      (define-key km "s" 'twitter2-scroll-mode)
      (define-key km "t" 'twitter2-toggle-proxy)
      (define-key km "\C-c\C-p" 'twitter2-toggle-proxy)
      (define-key km "\C-c\C-q" 'twitter2-suspend)
      nil))

(defun twitter2-keybind-message ()
  (let ((important-commands
	 '(("Timeline" . twitter2-friends-timeline)
	   ("Replies" . twitter2-replies-timeline)
	   ("Update status" . twitter2-update-status-interactive)
	   ("Next" . twitter2-goto-next-status)
	   ("Prev" . twitter2-goto-previous-status))))
...
;;  0.1 t
;;  '(lambda ()
;;     (when (equal (buffer-name (current-buffer)) twitter2-buffer)
;;       (message (twitter2-keybind-message)))))
...
;;   (add-to-list 'minor-mode-alist '(twitter2-jojo-mode " tw-jojo"))
  (setq twitter2-username-active twitter2-username)
  (setq twitter2-password-active twitter2-password)
  (when twitter2-use-convert
    (if (null twitter2-convert-program)
	(setq twitter2-use-convert nil)
      (with-temp-buffer
	(call-process twitter2-convert-program nil (current-buffer) nil
		      "-version")
	(goto-char (point-min))
	(if (null (search-forward-regexp "\\(Image\\|Graphics\\)Magick" nil t))
	    (setq twitter2-use-convert nil)))))
  (twitter2-setup-proxy)
  )
...
(defun twitter2-update-mode-line ()
  "Update mode line"
  (let ((enabled-options nil)
	(spec-string twitter2-last-retrieved-timeline-spec-string))
    (when twitter2-jojo-mode
      (push "jojo" enabled-options))
    (when twitter2-icon-mode
      (push "icon" enabled-options))
    (when twitter2-scroll-mode
      (push "scroll" enabled-options))
    (when twitter2-proxy-use
      (push "proxy" enabled-options))
    (when twitter2-use-ssl
      (push "ssl" enabled-options))
    (setq mode-name
	  (concat twitter2-mode-string
		  (if spec-string
		      (concat " " spec-string)
		    "")
		  (if enabled-options
...
(defun twitter2-find-curl-program ()
  ...
  (block nil
	...
	(cond 
	 ...
	 ((not (with-temp-buffer
      ...
      (if twitter2-use-ssl
	  (twitter2-start-http-ssl-session
...
(defvar twitter2-cert-file nil)
(defun twitter2-ensure-ca-cert ()
  "Create a CA certificate file if it does not exist, and return
its file name."
  (if twitter2-cert-file
      twitter2-cert-file
...
-----END CERTIFICATE-----"))
      (setq twitter2-cert-file file-name))))

(defun twitter2-start-http-ssl-session (curl-program method headers host port path parameters &optional noninteractive sentinel)
  ;; TODO: use curl
	  ...
	  `("--include" "--silent"
	    ,@(mapcan (lambda (pair)
			(list "-H"
		      ...
		      headers)
	    "--cacert"
	    ,(twitter2-ensure-ca-cert))))
    (when twitter2-proxy-use
      (nconc curl-args `("-x" ,(format "%s:%s" twitter2-proxy-server
					 twitter2-proxy-port)))
      (when (and twitter2-proxy-user
		 twitter2-proxy-password)
	(nconc curl-args `("-U" ,(format "%s:%s" twitter2-proxy-user
					   twitter2-proxy-password)))))
...
Available keywords:
  ...
  :query-string
  "
  (let* ((schema (if twitter2-use-ssl "https" "http"))
	 (default-port (if twitter2-use-ssl 443 80))
...
(defun twitter2-http-application-headers (&optional method headers)
  ...
  (let ((headers headers))
    ...
    (when twitter2-proxy-use
      (when twitter2-proxy-keep-alive
...
(defun twitter2-http-get (host method &optional noninteractive parameters sentinel)
  (if (null sentinel)
      (setq sentinel 'twitter2-http-get-default-sentinel))

  (twitter2-start-http-session
   "GET" (twitter2-http-application-headers "GET")
   host nil (concat "/" method ".xml") parameters noninteractive sentinel))

(defun twitter2-created-at-to-seconds (created-at)
  (let ((encoded-time (apply 'encode-time (parse-time-string created-at))))
    (+ (* (car encoded-time) 65536)
       (cadr encoded-time))))

(defun twitter2-http-get-default-sentinel (temp-buffer noninteractive proc stat &optional suc-msg)
  (debug-printf "get-default-sentinel: proc=%s stat=%s" proc stat)
  (unwind-protect
      (let ((header (twitter2-get-response-header temp-buffer))
	    (body (twitter2-get-response-body temp-buffer))
	    (status nil))
	(if (string-match "HTTP/1\.[01] \\([a-zA-Z0-9 ]+\\)\r?\n" header)
	    (when body
	      ...
	      (case-string
	       status
	       (("200 OK")
		(setq twitter2-new-tweets-count
		      (count t (mapcar
				#'twitter2-cache-status-datum
				(reverse (twitter2-xmltree-to-status
					  body)))))
		(setq twitter2-timeline-data
		      (sort twitter2-timeline-data
			    (lambda (status1 status2)
			      (let ((created-at1
				     (twitter2-created-at-to-seconds
				      (cdr (assoc 'created-at status1))))
				    (created-at2
				     (twitter2-created-at-to-seconds
				      (cdr (assoc 'created-at status2)))))
				(> created-at1 created-at2)))))
		(if (and (> twitter2-new-tweets-count 0)
			 noninteractive)
		    (run-hooks 'twitter2-new-tweets-hook))
		(setq twitter2-last-retrieved-timeline-spec-string
		      twitter2-last-requested-timeline-spec-string)
		(twitter2-render-timeline)
		(twitter2-add-timeline-history)
		(when twitter2-notify-successful-http-get
		  ))
	       (t (message status))))
...
(defun twitter2-http-get-list-index-sentinel (temp-buffer noninteractive proc stat &optional suc-msg)
  (debug-printf "get-list-index-sentinel: proc=%s stat=%s" proc stat)
  (unwind-protect
      (let ((header (twitter2-get-response-header temp-buffer)))
	(if (not (string-match "HTTP/1\.[01] \\([a-zA-Z0-9 ]+\\)\r?\n" header))
	    (setq twitter2-list-index-retrieved "Failure: Bad http response.")
	  (let ((status (match-string-no-properties 1 header))
		(indexes nil))
	    (if (not (string-match "\r?\nLast-Modified: " header))
		(setq twitter2-list-index-retrieved
		...
		(with-current-buffer temp-buffer
		  (save-excursion
		    ...
		    (if indexes
			(setq twitter2-list-index-retrieved indexes)
		      (setq twitter2-list-index-retrieved "")))))
	       (t
		(setq twitter2-list-index-retrieved status)))))))
    ;; unwindforms
    (when (and (not twitter2-debug-mode) (buffer-live-p temp-buffer))
      (kill-buffer temp-buffer)))
  )

(defun twitter2-http-post (host method &optional parameters contents sentinel)
...
PARAMETERS is alist of URI parameters.
 ex) ((\"mode\" . \"view\") (\"page\" . \"6\")) => <URI>?mode=view&page=6"
  (if (null sentinel)
      (setq sentinel 'twitter2-http-post-default-sentinel))

  (twitter2-start-http-session
   "POST" (twitter2-http-application-headers "POST")
   host nil (concat "/" method ".xml") parameters noninteractive sentinel))

(defun twitter2-http-post-default-sentinel (temp-buffer noninteractive proc stat &optional suc-msg)
  (debug-printf "post-default-sentinel: proc=%s stat=%s" proc stat)
  (unwind-protect
      (let ((header (twitter2-get-response-header temp-buffer))
	    ;; (body (twitter2-get-response-body temp-buffer)) not used now.
	    (status nil))
	(if (string-match "HTTP/1\.[01] \\([a-zA-Z0-9 ]+\\)\r?\n" header)
	    (setq status (match-string-no-properties 1 header))
	  (setq status
...
(defun twitter2-get-response-header (buffer)
...
(defun twitter2-get-response-body (buffer)
...
(defun twitter2-status-to-status-datum (status)
  (flet ((assq-get (item seq)
    ...
    (let* ((status-data (cddr status))
      ...
      (when (and retweeted-status-data twitter2-use-native-retweet)
	(setq original-user-screen-name (twitter2-decode-html-entities
					 (assq-get 'screen_name user-data))
	      original-user-name (twitter2-decode-html-entities
	...
	(when regex-index
      ...
      (when (or (null twitter2-timeline-last-update)
                (< (twitter2-created-at-to-seconds
                    twitter2-timeline-last-update)
                   (twitter2-created-at-to-seconds created-at)))
...
(defun twitter2-render-timeline ()
  (with-current-buffer (twitter2-buffer)
    (let ((point (point))
      ...
      (if (and twitter2-image-stack window-system)
	  (clear-image-cache))
...
(defun twitter2-make-display-spec-for-icon (image-url)
...
image are displayed."
  (let* ((image-data (twitter2-retrieve-image image-url))
	 (image-spec
	  `(image :type ,(car image-data)
		  :data ,(cdr image-data))))
    (if (not (image-type-available-p (car image-data)))
	nil
      (if (and twitter2-convert-fix-size (not twitter2-use-convert))
	  (let* ((size (if (cdr image-data)
			   (image-size image-spec t)
			 '(48 . 48)))
		 (width (car size))
		 (height (cdr size))
		 (fixed-length twitter2-convert-fix-size)
...
(defun twitter2-format-string (string prefix replacement-table)
...
variable `twitter2-status-format'"
  (flet ((attr (key)
	       (assocref key status))
	 (profile-image
	  ()
	  (let ((profile-image-url (attr 'user-profile-image-url))
		(icon-string "\n  "))
	    (unless (gethash
		     `(,profile-image-url . ,twitter2-convert-fix-size)
		     twitter2-image-data-table)
	      (add-to-list 'twitter2-image-stack profile-image-url))
	    
	    (when (and icon-string twitter2-icon-mode)
	      (let ((display-spec
		     (twitter2-make-display-spec-for-icon profile-image-url)))
		(when display-spec
	 ...
	 (make-string-with-url-property
	  ...
	  (let ((result (copy-sequence str)))
	    (add-text-properties
	     0 (length result)
	     `(mouse-face highlight face twitter2-uri-face uri ,url)
	     result)
	    result)))
    (let* ((replace-table
	    `(("%" . "%")
	       ...
	       ,(let* ((created-at
			...
			(cond
			 ...
			 (t (format-time-string "%I:%M %p %B %d, %Y"
						created-at))))
		       (url
			(twitter2-get-status-url (attr 'user-screen-name)
						   (attr 'id))))
		 ...
		 (let ((str (cdr (assq 'following-string context)))
		  ...
		  (if (or (string= "" reply-id) (string= "" reply-name))
		    ...
		    (let ((in-reply-to-string
			  ...
			  (url
			   (twitter2-get-status-url reply-name reply-id)))
		      (concat " "
			      (make-string-with-url-property
		  ...
		  (let* ((str (cdr (assq 'processed-string context)))
			 ...
			 (table (cdr (assq 'replacement-table context))))
		    (store-match-data match-data)
		    (let* ((postfix (twitter2-format-string
	   ...
	   (formatted-status
	    (twitter2-format-string format-str "%" replace-table)))
      (add-text-properties
...
(defun twitter2-show-minibuffer-length (&optional beg end len)
  ...
  (when (minibuffer-window-active-p (selected-window))
    ...
    (let* ((deactivate-mark deactivate-mark)
	   ...
	   (sign-len (length (twitter2-sign-string)))
	   (mes (if (< 0 sign-len)
		    (format "%d=%d+%d"
...
(defun twitter2-setup-minibuffer ()
  (add-hook 'post-command-hook 'twitter2-show-minibuffer-length t t))

(defun twitter2-finish-minibuffer ()
  (remove-hook 'post-command-hook 'twitter2-show-minibuffer-length t))

(defun twitter2-status-not-blank-p (status)
  (with-temp-buffer
    (insert status)
    (goto-char (point-min))
    ;; skip user name
...
(defun twitter2-update-status-from-minibuffer (&optional init-str reply-to-id)
  (when (and (null init-str)
	     twitter2-current-hashtag)
    (setq init-str (format " #%s " twitter2-current-hashtag)))
  (let ((status init-str)
	(sign-str (twitter2-sign-string))
	(not-posted-p t)
	(prompt "status: ")
	(map minibuffer-local-map)
	(minibuffer-message-timeout nil))
    (define-key map (kbd "<f4>") 'twitter2-tinyurl-replace-at-point)
    (when twitter2-use-show-minibuffer-length
      (add-hook 'minibuffer-setup-hook 'twitter2-setup-minibuffer t)
      (add-hook 'minibuffer-exit-hook 'twitter2-finish-minibuffer t))
    (unwind-protect
	(while not-posted-p
	  (setq status (read-from-minibuffer prompt status map nil 'twitter2-tweet-history nil t))
	  (let ((status-with-sign (concat status sign-str)))
	    (if (< 140 (length status-with-sign))
		(setq prompt "status (too long): ")
	      (progn
		(setq prompt "status: ")
		(when (twitter2-status-not-blank-p status)
		  (let ((parameters `(("status" . ,status-with-sign)
				      ("source" . "twmode")
				      ,@(if reply-to-id
					    `(("in_reply_to_status_id"
					       . ,reply-to-id))))))
		    (twitter2-http-post "twitter.com" "statuses/update" parameters)
		    (setq not-posted-p nil)))
		))))
      ;; unwindforms
      (when (memq 'twitter2-setup-minibuffer minibuffer-setup-hook)
	(remove-hook 'minibuffer-setup-hook 'twitter2-setup-minibuffer))
      (when (memq 'twitter2-finish-minibuffer minibuffer-exit-hook)
	(remove-hook 'minibuffer-exit-hook 'twitter2-finish-minibuffer))
      )))

(defun twitter2-get-list-index (username)
  (twitter2-http-get "api.twitter.com"
		       (concat "1/" username "/lists")
		       t nil
		       'twitter2-http-get-list-index-sentinel))

(defun twitter2-get-list-index-sync (username)
  (setq twitter2-list-index-retrieved nil)
  (twitter2-get-list-index username)
  (while (not twitter2-list-index-retrieved)
    (sit-for 0.1))
  (cond
   ((stringp twitter2-list-index-retrieved)
    (if (string= "" twitter2-list-index-retrieved)
	(message (concat username " has no list"))
      (message twitter2-list-index-retrieved))
    nil)
   ((listp twitter2-list-index-retrieved)
    twitter2-list-index-retrieved)))

(defun twitter2-manage-friendships (method username)
  (twitter2-http-post "twitter.com"
...
(defun twitter2-get-twits (host method &optional noninteractive id)
  (let ((buf (get-buffer twitter2-buffer)))
    (if (not buf)
	(twitter2-stop)
      (let* ((default-count 20)
	     (count twitter2-number-of-tweets-on-retrieval)
	     (count (cond
	     ...
	     (count (min (max 1 count)
			 twitter2-max-number-of-tweets-on-retrieval))
	...
	(if id
	  ...
	  (when twitter2-timeline-last-update
	...
	(twitter2-http-get host method
			     noninteractive parameters))))

  (if (and twitter2-icon-mode window-system
	   twitter2-image-stack)
      (mapc 'twitter2-retrieve-image twitter2-image-stack)
    ))

(defun twitter2-get-and-render-timeline (spec &optional noninteractive id)
  (let* ((original-spec spec)
	 (spec-string (if (stringp spec)
			  spec
			(twitter2-timeline-spec-to-string spec)))
	 (spec ;; normalized spec.
	  (twitter2-string-to-timeline-spec spec-string)))
    (when (null spec)
      (error "\"%s\" is invalid as a timeline spec"
	     (or spec-string original-spec)))
    (setq twitter2-last-requested-timeline-spec-string spec-string)
    (unless
	(and twitter2-last-retrieved-timeline-spec-string
	     (twitter2-equal-string-as-timeline
	      spec-string twitter2-last-retrieved-timeline-spec-string))
      (setq twitter2-timeline-last-update nil
	    twitter2-timeline-data nil))
    (if (twitter2-timeline-spec-primary-p spec)
	(let ((pair (twitter2-timeline-spec-to-host-method spec)))
	  (when pair
	    (let ((host (car pair))
		  (method (cadr pair)))
	      (twitter2-get-twits host method noninteractive id))))
      (let ((type (car spec)))
	(error "%s has not been supported yet" type)))))

(defun twitter2-retrieve-image (image-url)
  (let ((image-data (gethash `(,image-url . ,twitter2-convert-fix-size)
			     twitter2-image-data-table)))
    (when (not image-data)
      (let ((image-type nil)
	    (image-spec nil)
	    (converted-image-size
	     `(,twitter2-convert-fix-size . ,twitter2-convert-fix-size)))
	(with-temp-buffer
	  ...
	  (let ((coding-system-for-read 'binary)
	    ...
	    (setq image-type (twitter2-image-type image-url
						    (current-buffer)))
	    (setq image-spec `(image :type ,image-type
				     :data ,(buffer-string)))
	    (when (and twitter2-convert-fix-size twitter2-use-convert
		       (not
	      ...
	      (call-process-region 
	       (point-min) (point-max)
	       twitter2-convert-program
	       t t nil
	       (if image-type (format "%s:-" image-type) "-")
	       "-resize"
	       (format "%dx%d" twitter2-convert-fix-size
		       twitter2-convert-fix-size)
	       "xpm:-")
	      (setq image-type 'xpm))
	    (setq image-data `(,image-type . ,(buffer-string))))
	  (puthash `(,image-url . ,twitter2-convert-fix-size)
		   image-data
		   twitter2-image-data-table))))
    image-data))

(defun twitter2-tinyurl-get (longurl)
  "Tinyfy LONGURL"
  (let ((api (cdr (assoc twitter2-tinyurl-service
			 twitter2-tinyurl-services-map))))
    (unless api
      (error "Invaild `twitter2-tinyurl-service'. try one of %s"
...
(defun twitter2-start (&optional action)
  (interactive)
  (if (null action)
      (setq action #'twitter2-current-timeline-noninteractive))
  (if twitter2-timer
      nil
    (setq twitter2-timer
	  (run-at-time "0 sec"
...
(defun twitter2-friends-timeline ()
  (interactive)
  (twitter2-get-and-render-timeline '(friends)))

(defun twitter2-replies-timeline ()
  (interactive)
  (twitter2-get-and-render-timeline '(replies)))

(defun twitter2-public-timeline ()
  (interactive)
  (twitter2-get-and-render-timeline '(public)))

(defun twitter2-user-timeline ()
  (interactive)
  (twitter2-get-and-render-timeline `(user ,(twitter2-get-username))))

(defun twitter2-current-timeline-noninteractive ()
  (twitter2-current-timeline t))

(defun twitter2-current-timeline (&optional noninteractive)
  (interactive)
  (let ((spec (or twitter2-last-retrieved-timeline-spec-string
		  twitter2-initial-timeline-spec-string)))
    (twitter2-get-and-render-timeline spec noninteractive)))
...
(defun twitter2-set-current-hashtag (&optional tag)
  (interactive)
  (unless tag
    (setq tag (twitter2-completing-read "hashtag (blank to clear): #"
					  twitter2-hashtag-history
					  nil nil
					  twitter2-current-hashtag
					  'twitter2-hashtag-history))
    (message
     (if (eq 0 (length tag))
	 (progn (setq twitter2-current-hashtag nil)
		"Current hashtag is not set.")
       (progn
	 (setq twitter2-current-hashtag tag)
	 (format "Current hashtag is #%s" twitter2-current-hashtag))))))

(defun twitter2-erase-old-statuses ()
  (interactive)
  (setq twitter2-timeline-data nil)
  (if (not twitter2-last-retrieved-timeline-spec-string)
      (setq twitter2-last-retrieved-timeline-spec-string
	    twitter2-initial-timeline-spec-string)
    (let* ((spec-string twitter2-last-retrieved-timeline-spec-string)
	   (spec (twitter2-string-to-timeline-spec spec-string))
	   (pair (twitter2-timeline-spec-to-host-method spec))
	   (host (car pair))
	   (method (cadr pair)))
	...
	(let* ((system-time-locale "C")
		...
		(twitter2-global-strftime
		 "%a, %d %b %Y %H:%M:%S GMT"
		 twitter2-timeline-last-update)))
	  (twitter2-http-get host method nil `(("since" . ,since))))))))
...
(defun twitter2-tinyurl-replace-at-point ()
  "Replace the url at point with a tiny version."
  (interactive)
  (let ((url-bounds (bounds-of-thing-at-point 'url)))
    (when url-bounds
      (let ((url (twitter2-tinyurl-get (thing-at-point 'url))))
	(when url
	  (save-restriction
...
(defun twitter2-retweet ()
  (interactive)
  (if twitter2-use-native-retweet
      (twitter2-native-retweet)
    (twitter2-organic-retweet)))

(defun twitter2-organic-retweet ()
  (interactive)
  (let ((username (get-text-property (point) 'username))
	(text (get-text-property (point) 'text))
	(id (get-text-property (point) 'id))
	(retweet-time (current-time))
	(format-str (or twitter2-retweet-format
			"RT: %t (via @%s)")))
    (when username
      (let ((prefix "%")
	    (replace-table
	...
	(twitter2-update-status-from-minibuffer
	 (twitter2-format-string format-str prefix replace-table))
	))))
...
(defun twitter2-follow (&optional remove)
  (interactive)
  (let ((username (get-text-property (point) 'username))
	(method (if remove "destroy" "create"))
	(mes (if remove "unfollowing" "following")))
    (unless username
      (setq username (twitter2-read-username-with-completion
		      "who: " "" 'twitter2-user-history)))
    (if (> (length username) 0)
	(when (y-or-n-p (format "%s %s? " mes username))
	  (twitter2-manage-friendships method username))
      (message "No user selected"))))

(defun twitter2-unfollow ()
  (interactive)
  (twitter2-follow t))

(defun twitter2-native-retweet ()
  (interactive)
  (let ((id (get-text-property (point) 'id))
	...
	(let ((mes (format "Retweet \"%s\"? "
			   (if (> (length text) len)
			     ...
			     text))))
	  (when (y-or-n-p mes)
	    (twitter2-http-post "api.twitter.com"
...
(defun twitter2-visit-timeline (&optional timeline-spec initial)
  (interactive)
  (let ((timeline-spec
	 (or timeline-spec
	     (twitter2-read-timeline-spec-with-completion
	      "timeline: " initial t))))
    (when timeline-spec
      (twitter2-get-and-render-timeline timeline-spec))))

(defun twitter2-other-user-timeline ()
  (interactive)
  (let* ((username (get-text-property (point) 'username))
	 (screen-name-in-text
	  (get-text-property (point) 'screen-name-in-text))
	 (spec (cond (screen-name-in-text `(user ,screen-name-in-text))
		     ...
		     (t nil))))
    (if spec
	(twitter2-get-and-render-timeline spec)
      (message "No user selected"))))

(defun twitter2-other-user-timeline-interactive ()
  (interactive)
  (let ((username
	 (twitter2-read-username-with-completion
	  "user: " nil
	  'twitter2-user-history)))
    (if (> (length username) 0)
	(twitter2-get-and-render-timeline `(user ,username))
      (message "No user selected"))))

(defun twitter2-other-user-list-interactive ()
  (interactive)
  (let ((username (twitter2-read-username-with-completion
		   "whose list: "
		   (get-text-property (point) 'username)
		   'twitter2-user-history)))
    (if (string= "" username)
	(message "No user selected")
      (let* ((list-name (twitter2-read-list-name username))
	     (spec `(list ,username ,list-name)))
	(when list-name
	  (twitter2-get-and-render-timeline spec))))))

(defun twitter2-direct-message ()
  (interactive)
  (let ((username (get-text-property (point) 'username)))
    (if username
	(twitter2-update-status-from-minibuffer (concat "d " username " ")))))
...
(defun twitter2-make-list-from-assoc (key data)
  (mapcar (lambda (status)
	    (cdr (assoc key status)))
	  data))

(defun twitter2-read-username-with-completion (prompt init-user &optional history)
  (let ((collection
	 (append (twitter2-make-list-from-assoc
		  'user-screen-name twitter2-timeline-data)
		 twitter2-user-history)))
    (twitter2-completing-read prompt collection nil nil init-user history)))

(defun twitter2-read-list-name (username &optional list-index)
  (let* ((list-index (or list-index
			 (twitter2-get-list-index-sync username)))
	 (prompt (concat username "'s list: "))
	 (listname
	  (if list-index
	      (twitter2-completing-read prompt list-index nil t nil)
	    nil)))
    (if (string= "" listname)
	nil
      listname)))

(defun twitter2-read-timeline-spec-with-completion (prompt initial &optional as-string)
  (let* ((dummy-hist (append twitter2-timeline-history
			     (twitter2-make-list-from-assoc
			      'user-screen-name twitter2-timeline-data)))
	 (spec-string (twitter2-completing-read prompt dummy-hist
						  nil nil initial 'dummy-hist))
	 (spec-string
	  (if (string-match "^\\([^/]+\\)/$" spec-string)
	      (let* ((username (match-string 1 spec-string))
		     (list-index (twitter2-get-list-index-sync username))
		     (listname
		      (if list-index
			  (twitter2-read-list-name username list-index)
			nil)))
		(if listname
...
(defun twitter2-get-username ()
  (or twitter2-username-active
      (setq twitter2-username-active (read-string "your twitter username: "))))

(defun twitter2-get-password ()
  (or twitter2-password-active
      (setq twitter2-password-active (read-passwd "your twitter password: "))))
...
(defun twitter2-goto-next-status ()
  ...
  (let ((pos))
	...
	(goto-char pos)
      (let ((id (get-text-property (point) 'id)))
        (if id
	    (twitter2-get-and-render-timeline
	     twitter2-last-retrieved-timeline-spec-string
	     nil id))))))
...
(defun twitter2-goto-previous-status ()
  ...
  (let* ((current-pos (point))
         (prev-pos (twitter2-get-previous-username-face-pos current-pos)))
  ...
  (let ((prop))
	...
	(when (eq pos nil)
	  (let ((head-prop (get-text-property (point-min) 'face)))
	    (if (and
		 (not (eq prop twitter2-username-face))
		 (eq head-prop twitter2-username-face))
		(setq pos (point-min))
	      (throw 'not-found nil)
	      )))
	(setq prop (get-text-property pos 'face)))
...
(defun twitter2-goto-previous-status-of-user ()
  ...
  (let ((user-name (twitter2-get-username-at-pos (point)))
    ...
    (if (and pos
             (not (eq pos prev-pos))
             (equal (twitter2-get-username-at-pos pos) user-name))
	(goto-char pos)
      (if user-name
	  (message "Start of %s's status." user-name)
	(message "Invalid user-name.")))))

(defun twitter2-goto-next-thing (&optional backword)
			...
			(face-pred
			 (lambda (face)
			   (cond
			    ...
			    (t nil)))))
		   (member-if face-pred
			      '(twitter2-username-face
				twitter2-uri-face)))))
      (setq pos (funcall propety-change-f pos 'face)))
    (when pos
      (goto-char pos))))

(defun twitter2-goto-previous-thing (&optional backword)
  "Go to previous interesting thing. ex) username, URI, ... "
  (interactive)
  (twitter2-goto-next-thing (not backword)))
...
(defun twitter2-suspend ()
  "Suspend twitter2-mode then switch to another buffer."
  (interactive)
