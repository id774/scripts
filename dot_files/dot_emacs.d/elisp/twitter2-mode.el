...
(defvar twitter2-timer nil "Timer object for timeline refreshing will be stored here. DO NOT SET VALUE MANUALLY.")
...
(setq twitter2-status-format "%i %s: %t %p")
;; %s - screen_name
;; %S - name
;; %i - profile_image
;; %d - description
...
(defvar twitter2-friends-timeline-data nil)
(defvar twitter2-friends-timeline-last-update nil)
...
(defvar twitter2-icon-mode nil "You MUST NOT CHANGE this variable directory. You should change through function'twitter2-icon-mode'")
  ...
  (setq twitter2-icon-mode
	(if twitter2-icon-mode
	  ...
	  (when (or (null arg)
		...
		t)))))
  (twitter2-render-friends-timeline))
...
(defun twitter2-render-friends-timeline ()
  (with-current-buffer (twitter2-buffer)
    (let ((point (point))
      ...
      (mapc (lambda (status)
	      ...
	      (insert "\n"))
	    twitter2-friends-timeline-data)
	  ...
	  ((?@)                         ; %@ - X seconds ago
	   (let ((created-at
	     ...
	     (let ((secs (+ (* (- (car now) (car created-at)) 65536)
	       ...
	       (setq url (twitter2-get-status-url (attr 'user-screen-name) (attr 'id)))
...
(defun twitter2-cache-status-datum (status-datum &optional data-var)
  "Cache status datum into data-var(default twitter2-friends-timeline-data)
If STATUS-DATUM is already in DATA-VAR, return nil. If not, return t."
  (if (null data-var)
      (setf data-var 'twitter2-friends-timeline-data))
  (let ((id (cdr (assq 'id status-datum))))
	...
	(progn
	  (if twitter2-jojo-mode
	      (twitter2-update-jojo (cdr (assq 'user-screen-name status-datum))
...
(defun twitter2-status-to-status-datum (status)
  (flet ((assq-get (item seq)
	...
	(when regex-index
      ...
      (setq twitter2-friends-timeline-last-update created-at)
...
(defun twitter2-start (&optional action)
  (interactive)
  (if (null action)
      (setq action #'twitter2-friends-timeline))
  (if twitter2-timer
      nil
    (setq twitter2-timer
	  (run-at-time "0 sec"
...
(defun twitter2-friends-timeline ()
  ...
  (let ((buf (get-buffer twitter2-buffer)))
    (if (not buf)
	(twitter2-stop)
       (if (not twitter2-friends-timeline-last-update)
	   (twitter2-http-get "statuses" "friends_timeline")
	 (let* ((system-time-locale "C")
		(since
		  (twitter2-global-strftime
		   "%a, %d %b %Y %H:%M:%S GMT"
		   twitter2-friends-timeline-last-update)))
	   (twitter2-http-get "statuses" "friends_timeline"
				`(("since" . ,since)))))))
...
(defun twitter2-erase-old-statuses ()
  (interactive)
  (setq twitter2-friends-timeline-data nil)
  (if (not twitter2-friends-timeline-last-update)
      (twitter2-http-get "statuses" "friends_timeline")
    (let* ((system-time-locale "C")
	   (since
